...
const inputVariants = cva(
  'flex w-full rounded-lg border transition-all duration-200 file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-gray-500 dark:placeholder:text-gray-400 disabled:cursor-not-allowed disabled:opacity-50',
  {
    variants: {
      variant: {
    ...
    return (
      <div className="w-full">
        ...
        <div className="relative">
          ...
          <input
            className={inputVariants({ variant, size, error: !!error, className: leftIcon ? 'pl-10' : rightIcon ? 'pr-10' : '', className })}
            ref={ref}
            {...props}
          />
          {rightIcon && (
